...
setTextLineTrigger change_db :change_array "Deployed Fighters"
                #   if ($tracker_status = "ON")
                #      setTextLineTrigger auto_report :parse_fig_line "Deployed Fighters"
                #   end
pause
...
getWord CURRENTLINE $test_change 1
        if ($test_change <> "Deployed")
           ...
           setVar $figArray[$dead_sector] 0
                  if ($tracker_status = "ON")
                     if ($last_hit = $dead_sector)
                        goto :main_2
                     end
                 #    setVar $last_hit $dead_sector
                     setVar $input $dead_sector
                     setVar $last_hit $input
                     goto :isnum
                  end
...
:input
killalltriggers
echo ansi_14 "*Enter desired Sector (commands : v, uv, va, de, p, fp, tracker)*"
...
:readcomm
  getWord $input $request 1
    ...
    elseif ($request = "fp")
      ...
      goSub :breadth_search
      goto :echo
    elseif ($request = "tracker")   
           if ($tracker_status = "OFF")
                      setVar $tracker_status "ON"
           else
                      setVar $tracker_status "OFF"
           end
           goto :main_2
