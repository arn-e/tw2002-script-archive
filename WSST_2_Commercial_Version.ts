...
fileExists $exists $bust_list
           ...
           if ($bust_exist = "YES")
              echo ansi_15 "*Would you like to clear your avoids ? (" ansi_12 "Y" ansi_15 "/" ansi_12 "N" ansi_15 ") : "
...
echo ansi_15 "*Checking Auth Info..."
if (STARDOCK <> 4008)
   halt
end
     :authentication
     ...
     getWord CURRENTLINE $location 1 
             if ($location <> "Command")
                echo ansi_15 "*Not at Command Prompt - Halting*"
                halt
             end
             send "v"
             ...
             stripText $stardock "."
             #echo ansi_14 "*" $stardock "*"
                     if ($stardock <> 4008)
                        echo ansi_15 "*Script not Authorized for use in this Game"
                        echo ansi_15 "*Contact : " ansi_12 "$playerName " ansi_15 "for more Information*"
                        halt
                     end
                     send "i"
                          waitfor "Trader Name"
                          getWord CURRENTLINE $trader_name 5
                          if ($trader_name <> "phuquedup")
                             echo ansi_15 "*Script not Autherized for use with this Name*"
                             halt
                          end
...
echo ansi_15 "Credits : Thanks to SupG for Menu code**"
...
echo ansi_14 "3." ansi_15 " Buy Figs          : " ansi_9 "[" ansi_6 $restock ansi_9 "]*"
echo ansi_14 "4." ansi_15 " Ship #1 (current) : " ansi_9 "[" ansi_6 $this_ship ansi_9 "]*"
echo ansi_14 "5." ansi_15 " Ship #2 (other)   : " ansi_9 "[" ansi_6 $other_ship ansi_9 "]*"
...
getDistance $dist $bust_sector $tow_target
            if ($dist > 1)
               ...
               send " m " $short_course[1] " * z n m " $short_course[2] " * z n m " $short_course[3] " * z n "
               goto :lock_move
...
getWord CURRENTLINE $holds_needed 10
        if ($restock = "YES")
           ...
           setVar $avail_credits ($credits - 400000)
...
#OPTIONAL ADD ON : BUY FIGS AND SHIELDS
#Right now Just aiming at finishing off the routine
#Will add Frills like Fig / Shield Buy later
